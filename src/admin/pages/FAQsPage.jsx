...
export default function FAQsPage() {
  ...
  async function load() {
    setLoading(true);
    try { const d = await api.getFAQs(); setItems(d.data || d.faqs || []); } catch { setItems([]); }
    setLoading(false);
  }
  ...
  function openEdit(item) { setForm({ question: item.question || item.q || '', answer: item.answer || item.a || '', display_order: item.display_order ?? 0 }); setModal(item._id); }

  async function handleSave(e) {
    e.preventDefault(); setSaving(true);
    try {
      if (modal === 'add') { await api.createFAQ(form); showToast('Created'); }
      else { await api.updateFAQ(modal, form); showToast('Updated'); }
      setModal(null); load();
  ...
  async function handleDelete(id) {
    ...
    try { await api.deleteFAQ(id); showToast('Deleted'); load(); } catch (err) { showToast(err.message, 'error'); }
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-white text-2xl font-black">FAQs</h1>
      ...
      {loading ? <div className="flex items-center justify-center py-20"><div className="w-8 h-8 border-2 border-teal-500 border-t-transparent rounded-full animate-spin" /></div> : (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.length === 0 && <tr><td colSpan={4} className="text-center text-gray-500 py-12">No FAQs found</td></tr>}
              {items.map(item => (
                <tr key={item._id} className="border-b border-gray-800 hover:bg-gray-800/50">
                  <td className="px-4 py-3 text-white font-semibold max-w-[450px] truncate">{item.question || item.q}</td>
      ...
      {modal && (
        <Modal title={modal === 'add' ? 'Add FAQ' : 'Edit FAQ'} onClose={() => setModal(null)}>
          <form onSubmit={handleSave} className="space-y-3">
            <div><label className="block text-gray-400 text-xs font-bold mb-1 uppercase">Question</label><input className={inp} value={form.question} onChange={e => f('question', e.target.value)} required /></div>
            <div><label className="block text-gray-400 text-xs font-bold mb-1 uppercase">Answer</label><textarea className={inp} rows={4} value={form.answer} onChange={e => f('answer', e.target.value)} required /></div>

...
const empty = { name: '', count: 0, icon: '', color: '', display_order: 0 };
...
export default function DepartmentsPage() {
  ...
  async function load() {
    setLoading(true);
    try { const d = await api.getDepartments(); setItems(d.data || d.departments || []); } catch { setItems([]); }
    setLoading(false);
  }
  ...
  function openEdit(item) { setForm({ name: item.name || '', count: item.count ?? 0, icon: item.icon || '', color: item.color || '', display_order: item.display_order ?? 0 }); setModal(item._id); }

  async function handleSave(e) {
    e.preventDefault(); setSaving(true);
    try {
      if (modal === 'add') { await api.createDepartment(form); showToast('Created'); }
      else { await api.updateDepartment(modal, form); showToast('Updated'); }
      setModal(null); load();
  ...
  async function handleDelete(id) {
    ...
    try { await api.deleteDepartment(id); showToast('Deleted'); load(); } catch (err) { showToast(err.message, 'error'); }
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-white text-2xl font-black">Departments</h1>
      ...
      {loading ? <div className="flex items-center justify-center py-20"><div className="w-8 h-8 border-2 border-teal-500 border-t-transparent rounded-full animate-spin" /></div> : (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.length === 0 && <tr><td colSpan={5} className="text-center text-gray-500 py-12">No departments found</td></tr>}
              {items.map(item => (
                <tr key={item._id} className="border-b border-gray-800 hover:bg-gray-800/50">
                  ...
                  <td className="px-4 py-3 text-teal-400 font-bold">{item.count}</td>
      ...
      {modal && (
        <Modal title={modal === 'add' ? 'Add Department' : 'Edit Department'} onClose={() => setModal(null)}>
          <form onSubmit={handleSave} className="space-y-3">
            ...
            <div><label className="block text-gray-400 text-xs font-bold mb-1 uppercase">Count (staff)</label><input type="number" className={inp} value={form.count} onChange={e => f('count', Number(e.target.value))} /></div>
            ...
            <div><label className="block text-gray-400 text-xs font-bold mb-1 uppercase">Color</label><input className={inp} value={form.color} onChange={e => f('color', e.target.value)} placeholder="teal" /></div>
